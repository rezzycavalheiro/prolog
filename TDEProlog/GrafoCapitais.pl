...
% "profundidadeMax" retorna as listas com os caminhos até um determinado
% ponto escolhido pelo usuário. Para consultar, primeiro alterar o
% destino final em final(N) e então:
% ?- profundidadeMax(curitiba,R,4).
% "profundidade" retorna listas de caminhos possíveis entre dois
% vértices, repetindo os vértices. Por isso é interessante usar o
...
% Retorna uma lista com os vértices, em ordem crescente de tamanho de
% lista. Para consultar, primeiro alterar o destino final em final(N) e
% então: ?- largura(portoalegre,_,L).
...
% Encontra o último elemento de uma lista. Exemplo:
% ?- ultimo([a,b,c,d,e],X).
% X = e.
...
% Insere um elemento na última posição da lista. Exemplo:
...
% Retorna a distância entre um nó inicial e o final(N) escolhido, pela
% profundidade.
% Exemplo:
...
% X = 3 ;
% Isto siginifica que a distância de curitiba até vitoria, na última
% lista, é 3 (3 arestas).
...
% Cálculo da distância por largura. Exemplo:
